...
async function verifyAuth(req, env) {
  ...
  const record = await env.bottlesMeta.get(`user:${username}`, "json");
...
app.post("/register", async (c) => {
  const { username, passphrase_hash, pubkey, encrypted_privkey } = await c.req.json();
...
app.post("/login", async (c) => {
  ...
  const record = await c.env.bottlesMeta.get(`user:${username}`, "json");
...
app.get("/user/:username", async (c) => {
  const record = await c.env.bottlesMeta.get(
...
app.get("/bottles/:username", async (c) => {
  ...
  const metas = await Promise.all(
    list.objects.map(async (obj) => {
      const raw2 = await c.env.R2.get(obj.key);
      if (!raw2) return null;
      const b = await raw2.json();
